...
def predict():
    ...
    if file:
        ...
        try:
            ...
            # Resize original image to match input size (same as visualize_cam.py)
            ...
            return jsonify({
                'predicted_class': cfg.classes[pred_idx].replace('_', ' '),
                'confidence': f"{confidence:.2f}",
                'probabilities': {cfg.classes[i].replace('_', ' '): float(probs[i]) * 100 for i in range(len(cfg.classes))},
